...
einsteintoolkit/carpetx:cpu-real64 to reduce build time for CarpetX
dependencies.

To run them in a local copy of the container image do:

```
sudo docker pull einsteintoolkit/carpetx:cpu-real64
sudo docker run -it einsteintoolkit/carpetx /bin/bash -i

git clone -b main https://github.com/chcheng3/canudax-tests.git scripts
git clone -b gh-pages https://github.com/chcheng3/canudax-tests.git gh-pages
...
bitbucket of a user that can trigger the workflow.
...
pipelines:
  default:
    - step:
        ...
        script:
          - >
            curl -H "Accept: application/vnd.github.everest-preview+json" -H "Authorization: token $TRIGGER_TOKEN" --request POST --data '{"event_type": "webhook", "client_payload": {"trigger":"'$BITBUCKET_COMMIT'"}}' https://api.github.com/repos/chcheng3/canudax-tests/dispatches
```
